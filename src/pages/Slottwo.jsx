...
const Slottwo = () => {
  const [slots, setSlots] = useState([cricketTypes[2].logo, cricketTypes[2].logo, cricketTypes[2].logo]);
      ...
      <div id="slot-box" className="bg-gray-900 rounded-3xl p-10 shadow-2xl w-full max-w-3xl transition-all duration-500">
        ...
        <div className={`flex gap-2 mb-10 justify-center ${spinning ? "scroll-effect" : ""}`}>
          {slots.map((item, index) => (
            <div key={index} className="h-32 sm:h-20 md:p-5 md:h-50 w-full bg-gray-800 rounded-2xl flex items-center justify-center shadow-inner">
              <img src={item} alt={`Team ${index}`} className="w-[90%] h-full object-contain" />
            </div>
          ))}
        </div>
